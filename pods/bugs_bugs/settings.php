<?
	...
	$POD->registerPOD(
		...
		array("{$content_type}_document_editpath"=>$edit_link)		// global variables
	);
	
	// this pod creates a list of most recent content
	$POD->registerPOD(
		"contenttype_{$content_type}_browse",									// name
		"Create default list page for $content_type",				// description
		array("^{$permalink}/browse/(.*)$"=>$pod_dir . '/list.php?mode=$1'),				// rewrite rules
		array()														// global variables
	);
	
	$POD->registerPOD(
		"contenttype_{$content_type}_feeds",									// name
		"Create feeds for $content_type",				// description
		array("^{$permalink}/feeds/(.*)$"=>$pod_dir . '/browse_feed.php?mode=$1'),				// rewrite rules
		array()														// global variables
	);
	

	$POD->registerPOD(
		"contenttype_{$content_type}_bugfeeds",									// name
		"Create bug feeds for $content_type",				// description
		array("^{$permalink}/(.*)/feed"=> $pod_dir . '/bug_feed.php?stub=$1'),	// rewrite rules
		array()		// global variables
	);
	...
	$POD->registerPOD(
		...
		array("{$content_type}_document_path"=>$permalink)		// global variables
	);
