...
(function($) {
	...
	jQuery.fn.tagsInput = function(options) { 
		...
		var settings = jQuery.extend({width:'300px',height:'100px','hide':true,'delimiter':','},options);
		...
		this.each(function() { 
			...
			$('<div id="'+id+'_tagsinput" class="tagsinput"><div id="'+id+'_addTag"><input id="'+id+'_tag" value="" default="new tag" /></div><div class="tags_clear"></div></div>').insertAfter(this);
			...
			if ($(data.real_input).val()!='') { 
				...
				$(data.fake_input).css('color','#CCCCCC');				
			}
			...
			if (settings.autocomplete_url != undefined) { 
				$(data.fake_input).autocomplete(settings.autocomplete_url,{selectFirst:false}).bind('result',data,function(event,data,formatted) { 
					if (data) {
						d = data + "";	
				...
				$(data.fake_input).bind('blur',data,function(event) { 
					...
					$(event.data.fake_input).css('color','#CCCCCC');
					return false;
				});
